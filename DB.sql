...
CREATE TABLE public.machines (
  ...
  localisation text,
  etat text NOT NULL DEFAULT 'opérationnel'::text CHECK (etat = ANY (ARRAY['En service'::text, 'À l''arrêt'::text, 'En panne'::text, 'Consigné'::text, 'Hors service'::text])),
  puissance text,
  tension text,
  manuel_url text,
...
CREATE TABLE public.ordres_travail (
  ...
  statut character varying DEFAULT 'prévu'::character varying CHECK (statut::text = ANY (ARRAY['prévu'::character varying::text, 'en_cours'::character varying::text, 'terminé'::character varying::text, 'annulé'::character varying::text, 'clôturé_avec_anomalie'::character varying::text])),
  observations text,
  created_at timestamp with time zone DEFAULT now(),
  numot integer,
  ...
  intervention_source_id uuid,
  raison_report text,
  date_report timestamp with time zone,
  etapes_non_conformes jsonb,
  etapes_reportees jsonb,
  etapes_deja_faites jsonb,
  lot_defaillance text,
  famille_probleme text,
  mode_defaillance text,
  action_recommandee text,
  gravite_libelle text,
  gravite_classe integer,
  occurrence_libelle text,
  occurrence_classe integer,
  detectabilite_libelle text,
  detectabilite_classe integer,
  rpn integer,
  date_expression date,
  CONSTRAINT ordres_travail_pkey PRIMARY KEY (id),
  CONSTRAINT ordres_travail_intervention_source_fkey FOREIGN KEY (intervention_source_id) REFERENCES public.interventions(id),
...
END,
  ...
  etat_machine_apres text CHECK (etat_machine_apres = ANY (ARRAY['En service'::text, 'À l''arrêt'::text, 'En panne'::text, 'Consigné'::text, 'Hors service'::text])),
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  client_valide boolean NOT NULL DEFAULT false,
  commentaire_client text,
  action_cloturee boolean NOT NULL DEFAULT false,
  date_cloture_action date,
...
CREATE TABLE public.plan_action_lots (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  nom text NOT NULL UNIQUE CHECK (btrim(nom) <> ''::text),
  actif boolean NOT NULL DEFAULT true,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT plan_action_lots_pkey PRIMARY KEY (id)
);
CREATE TABLE public.plan_action_problem_families (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  lot_id uuid NOT NULL,
  nom text NOT NULL CHECK (btrim(nom) <> ''::text),
  actif boolean NOT NULL DEFAULT true,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT plan_action_problem_families_pkey PRIMARY KEY (id),
  CONSTRAINT plan_action_problem_families_lot_id_fkey FOREIGN KEY (lot_id) REFERENCES public.plan_action_lots(id)
);
CREATE TABLE public.plan_action_failure_modes (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
